...
$(document).ready(() => {
    ...
    //// ALLY SETTING UP ////
    // let isLoaded = false
    const ally = new AllyModel( 100, false, true )
    ally.load("models/ally_tris.js", ()=>{
        ally.position.set(200,0,-200)
        ally.move(new THREE.Vector3(
            200,
            0,
            -200
        ))
    ...
    function render ( ) {
        ...
        player.render ( )
        // if (isLoaded)
            ally.render ( delta, player._isMoving)
        // console.log(delta)
        for (let i in allies){
            allies[i].move(player.position)
            // allies[i].render( delta )
        }
